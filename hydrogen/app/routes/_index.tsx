...
import { resolvePairOffers, createPairBundleItems, type PairOffer } from '~/data/offers';
import {
  formatPrice,
  formatInr,
  getPriceInINR,
...
export const meta: Route.MetaFunction = () => {
  return [
    { title: 'Avirena Jewels – Anti-Tarnish Dailywear Jewelry India' },
    {
      name: 'description',
      content:
        'Discover anti-tarnish dailywear jewelry crafted in high-grade brass with surgical steel posts. Shop earrings, rings, and curated duo suites with free express delivery across India.',
    },
    {
      property: 'og:title',
      content: 'Avirena Jewels – Anti-Tarnish Dailywear Jewelry India',
    },
    {
      property: 'og:description',
      content:
        'Shop modern architectural anti-tarnish earrings and jewelry suites from Mumbai.',
    },
    {
      tagName: 'link',
      rel: 'canonical',
      href: 'https://avirenajewels.com',
    },
  ];
};
...
export default function Homepage() {
  ...
  const orgSchema = {
    ...
    logo: 'https://avirenajewels.com/logo.webp',
    contactPoint: {
      '@type': 'ContactPoint',
      telephone: '+91-9225261659',
      contactType: 'customer service',
      areaServed: 'IN',
      availableLanguage: ['en', 'hi'],
    },
    sameAs: ['https://www.instagram.com/avirenajewels/'],
  };
  ...
  const webSiteSchema = {
    ...
    potentialAction: {
      '@type': 'SearchAction',
      target: 'https://avirenajewels.com/search?q={search_term_string}',
      'query-input': 'required name=search_term_string',
    },
  };

  return (
    <div className="pb-20 overflow-hidden font-sans-body w-full text-[#413C23] bg-[#E7E4D5] select-none">
      <script
      ...
      <section className="relative w-full bg-[#E7E4D5] border-b border-[#D8D2C2] min-h-[calc(100vh-6rem)] sm:min-h-[calc(100vh-5rem)] min-h-[calc(100dvh-6rem)] sm:min-h-[calc(100dvh-5rem)] max-h-[1080px] flex flex-col justify-between p-4 pb-5 sm:p-10 md:p-12 lg:px-16 lg:py-10 select-none overflow-hidden">
        ...
        <div className="relative my-auto py-4 sm:py-12 md:py-16 flex items-center justify-center w-full z-10 overflow-visible px-2 sm:px-4">
          {/* Official Brand Logo */}
          <div className="w-full flex items-center justify-center select-none pointer-events-none z-0">
            <picture>
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 translate-y-[10%] sm:-translate-y-[10%] md:-translate-y-[22%] lg:-translate-y-[28%] z-20 pointer-events-auto">
            <HeroBaroquePearlRing onClick={() => navigate('/shop/rings')} />
          </div>
        </div>

        {/* Bottom Hero Bar */}
        <div className="w-full flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 sm:gap-6 z-10 pt-4 border-t border-[#D8D2C2]">
            ...
            <div className="duo-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {pairOffers.slice(0, 4).map((offer) => {
                ...
                return (
                  <div
                    key={offer.id}
                    onClick={() => navigate(`/products/${bundleTarget.handle || bundleTarget.id}`)}
      ...
      {/* 3. CURATED COLLECTION ("Designing, Crafting & Layering.") */}
      <section className="w-full bg-[#E7E4D5] py-16 sm:py-24 border-b border-[#D8D2C2] px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20">
        <div className="w-full space-y-8 sm:space-y-12">
      ...
      <section className="w-full bg-[#E7E4D5] py-16 sm:py-24 border-b border-[#D8D2C2] px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20">
        <div className="w-full space-y-8 sm:space-y-12">
          ...
          <div className="w-full h-64 sm:h-80 md:h-96 lg:h-[460px] xl:h-[500px] rounded-xs overflow-hidden border border-[#D8D2C2] relative bg-[#413C23] shadow-xs">
            <img
      ...
      <section className="w-full bg-[#E7E4D5] py-16 sm:py-24 border-b border-[#D8D2C2] px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 select-none">
        <div className="w-full space-y-8 sm:space-y-12">
          ...
          {/* Craftsmanship Highlights Strip */}
      ...
      <section className="silver-section w-full bg-[#E7E4D5] py-16 sm:py-24 border-b border-[#D8D2C2] px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 select-none">
        <div className="w-full space-y-8 sm:space-y-12">
          ...
          <div className="silver-grid grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {silverProducts.slice(0, 4).map((product) => {
              if (!product || !product.id) return null;
              const imageSrc = getProductImage(product);
              const wishlisted = isWishlisted(product.id);
              const isQuickAdding = quickAddedId === product.id;
      ...
      {/* 7. FEATURED PIECE SECTION (50/50 Editorial Canvas) */}
      {spotlightProduct && (
        <section className="w-full bg-[#E7E4D5] border-t border-b border-[#D8D2C2] select-none overflow-hidden">
          <div className="w-full grid grid-cols-1 md:grid-cols-2 items-stretch">
            ...
            <div
              onClick={() => navigate(`/products/${spotlightProduct.handle || spotlightProduct.id}`)}
      ...
      {/* 8. CURATED GIFTING & OCCASION HUB */}
      <section className="w-full bg-[#E7E4D5] py-16 sm:py-24 border-b border-[#D8D2C2] px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20">
              ...
              return (
                ...
                >
                  <div className="relative aspect-square w-full bg-[#FAF8F5] p-6 flex items-center justify-center overflow-hidden">
                    ...
                    >
                      {isQuickAdding ? <Check className="w-4 h-4 text-white" /> : <ShoppingBag className="w-4 h-4" />}
                    </button>
                  </div>
                  ...
                  <div className="p-4 space-y-1.5">
                    ...
                    <div className="flex items-center justify-between text-xs pt-0.5 flex-wrap gap-1">
                      {(() => {
                        ...
                        return (
                          <div className="flex items-baseline gap-2 flex-wrap">
                            <span className="text-base sm:text-lg font-bold text-black tracking-tight">{formatPrice(product.price || 0)}</span>
                            {comparePrice > (product.price || 0) && (
                              <>
                                <span className="text-xs text-[#991B1B] line-through font-normal">
                                  {formatPrice(comparePrice)}
