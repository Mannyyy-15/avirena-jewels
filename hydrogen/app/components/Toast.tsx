...
const ToastItem: React.FC<{ toast: ToastMessage; onDismiss: () => void }> = ({
  ...
  const getIcon = () => {
    switch (toast.type) {
      ...
      case 'wishlist':
        return <Heart className="w-4 h-4 text-[#D4AF37] fill-[#D4AF37]" />;
      default:
  ...
  return (
    <div className="pointer-events-auto bg-[#2C2C2A] text-white p-4 rounded-xs shadow-xl border border-[#D4AF37]/30 flex items-start gap-3 animate-in slide-in-from-bottom-2 fade-in duration-300">
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        {toast.actionLabel && toast.onAction && (
          <button
            ...
            className="mt-2 text-[11px] font-medium text-[#D4AF37] hover:underline uppercase tracking-wider block cursor-pointer"
          >
            {toast.actionLabel} →
          </button>
        )}
